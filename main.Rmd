...
### (c) Log likelihood of the OLS model
...
### (d) Spatial autocorrelation of residuals
...
Next, lets go into the spatial lag and spatial error regression models. The model summary is presented below:
...
The Breusch-Pagan test and Koenker-Bassett test above show that the spatial lag model still faces the problem of heteroscedasticity, as both p-values are lower than the 0.05 threshold.
...
Let's compare the spatial lag model with the original OLS model in terms of goodness of fit. The Akaike Information Criterion for the spatial lag model is 525.48, lower than the AIC for the OLS model, which is 1435. This indicates that the spatial lag model is a better fit for the data. In terms of log likelihood, the OLS model produced a log likelihood of -711, where the spatial lag model had a log likelihood of -255. The likelihood ratio test produced a p-value close to zero, also indicating that the spatial lag model fits the data better.
...
By plotting the residuals against lagged residuals, we can identify no spatial autocorrelation of the residuals anymore.
