...
md_housing_value_i = \beta\*{i0}+\beta{i1}\cdot log_n\_poverty_i+\beta\*{i2}\\cdot pct_bachelor_i+\beta{i3}\cdot pct_single_i+\beta\_{i4}\cdot pct_vacant_i+\epsilon\_i
...
For the Fixed Bandwidth method, the optimized bandwidth is 2863.5 ft. For the Adaptive Bandwidth Method, the bandwidth should capture about 13 (proportion of observations \* observations = 0.008130619 \* 1720 = 13.98466) nearest neighbors.
...
ggplot(df) +
    ...
    scale_fill_continuous(type = "viridis",
                      ...
                      name = "") +
    labs(title = "Local R-Square") +
    map_theme(title_size = 9, tick_size = 7) +
    theme(legend.position = c(0.8, 0.25))

```

-   Standardized coefficients ($\beta/SE$)

The following maps plotted the standardized coefficient ($\beta/SE$)for each independent variable. When the absolute value of the ratio between coefficient and standard error is no less than 2 ($|\beta/SE|\geq 2$), we can assume the relationship between the dependent and independent variable is significant.
...
#   var = choropleth_var_list[i]
#   this_sf = df_long %>% filter(., Legend == var) %>% 
#     mutate(category = case_when(paste0(var) < 0.6 ~ "Less than 0.6")) %>% 
#   map_list[[var]] = 
#     ggplot(this_sf) +
#     geom_sf(aes(fill = Value), color = NA) +
#     scale_fill_continuous(type = "viridis",
